...
log_path = os.path.dirname(os.path.realpath(__file__)) + "//loggers"  # log_path是存放日志的路径
if not os.path.exists(log_path):
    os.mkdir(log_path)  # 如果不存在这个loggers文件夹，就自动创建一个
...
class Log:
    ...
    def handle_logs(self):
        """处理日志过期天数和文件数量"""
        dir_list = ['loggers']  # 要删除文件的目录名
        for dir in dir_list:
            dirPath = os.path.dirname(os.path.realpath(__file__)) + '\\' + dir  # 拼接删除目录完整路径
